...
function validate_form()
{
    ...
    // 1.氏名のバリデーション
    $input['name'] = trim($_POST['name'] ?? ""); //POSTが設定されていないときに備えて、null合体演算子で対応
    ...
    // 4.コメントのバリデーション
    $input['comments'] = trim($_POST['comments'] ?? ""); //POSTが設定されていないときに備えて、null合体演算子で対応
    if (
        strlen($input['comments']) == 0
    ) { //入力されているかの確認
        $form_errors['comments'] = "コメントを入力して下さい";
    }

    // 5.パスワードのバリデーション
    $input['password'] = trim($_POST['password'] ?? ""); //POSTが設定されていないときに備えて、null合体演算子で対応
...
<head>
    <meta charset="UTF-8">
    <meta name="description" content="入力フォームです">
